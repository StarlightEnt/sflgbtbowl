...
export async function saveSeasonSetup({
  ...
  try {
    ...
    // League Setup (a separate admin step — see components/Admin/LeagueSetupForm.js)
    // is now the only place a league gets created or renamed. This function
    // only ever creates a season under an already-existing league, chosen
    // from the dropdown in the Season Setup form.
    const leagueCheck = await client.query(`SELECT id FROM leagues WHERE id = $1`, [leagueId]);
    if (leagueCheck.rows.length === 0) {
      throw new ValidationError("Selected league not found — run League Setup first");
    }
