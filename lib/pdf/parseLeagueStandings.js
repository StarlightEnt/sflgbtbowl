...
//
// The suffix must match one of these known abbreviations, not an
// arbitrary run of letters ([A-Za-z]* previously) — a real hyphenated
// surname (e.g. "Garcia-Lopez") also matches "any letters after a
// hyphen", and an unmatched suffix here just stays attached to the
// name (visible, correctable) instead of silently deleting a real
// name segment. If a real title abbreviation shows up that isn't
// listed, add it here rather than widening back to a catch-all.
...
function resolveTeamNumber(nameToNumber, rawName) {
  ...
  // Two team names sharing this prefix (e.g. "Pity Party" / "Pity Party
  // II") makes the match ambiguous — return undefined rather than
  // silently guessing the first one in insertion order; callers already
  // treat undefined as "skip this row", same as a pure miss.
...
export function parseLeagueStandingsPDF(text) {
  ...
  const standingsRows = [];
  {
    ...
    for (const line of lines) {
      ...
      const rowMatch = line.match(/^\d+\s*([A-Za-z].*)$/);
      if (!rowMatch) continue;
      const rest = rowMatch[1];

      // Splitting name from stats at the first digit breaks when the team
      // name itself ends in a digit (e.g. "Pity Party of 4") — the digit
      // gets swallowed into the name's boundary and the won/lost/pct
      // block parses one character short. The pct's decimal point is a
      // more reliable anchor: won+lost+pct-integer is always exactly the
      // 4 digits right before it. Fall back to the old first-digit split
      // for rows pdf-parse renders without a decimal point at all
      // (observed on the BYE row).
      const dotIdx = rest.indexOf(".");
      let team_name, tail;
      if (dotIdx >= 4 && /^\d{4}$/.test(rest.slice(dotIdx - 4, dotIdx))) {
        team_name = rest.slice(0, dotIdx - 4).trim();
        tail = rest.slice(dotIdx - 4);
      } else {
        const fallback = rest.match(/^([A-Za-z].*?)(\d.*)$/);
        if (!fallback) continue;
        team_name = fallback[1].trim();
        tail = fallback[2];
      }
