...
export function parseLeagueStandingsPDF(text) {
  ...
  const team_standings = standingsRows
    .map((row) => {
      const team_number = nameToNumber.get(row.team_name);
  ...
  const weekly_results = [];
  {
    ...
    for (const line of lines) {
      ...
      const team_a_number = nameToNumber.get(teamAName.trim());
      const team_b_number = nameToNumber.get(teamBName.trim());
