...
export async function parseLeagueStandingsPDF(buffer) {
  ...
  // Team Rosters is the sole canonical source of team_number <-> team_name.
  // Other sections' printed names (possibly truncated) are only lookup
  // keys for resolveTeamNumber, never the name that gets returned.
  const nameToNumber = new Map(teams.map((t) => [t.team_name, t.team_number]));
  const numberToName = new Map(teams.map((t) => [t.team_number, t.team_name]));
  ...
  if (standingsStart !== -1 && standingsEnd !== -1) {
    ...
    for (let i = standingsStart + 1; i < standingsEnd; i++) {
      ...
      team_standings.push({
        team_number,
        team_name: numberToName.get(team_number),
        points_won,
        points_lost,
        pct_won,
      });
